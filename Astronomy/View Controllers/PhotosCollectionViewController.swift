...
class PhotosCollectionViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    private func loadImage(forCell cell: ImageCollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        filterOp.addDependency(fetchOp)// = Filter op will not run until fetchOp is completed
        cacheOp.addDependency(filterOp)
        completionOp.addDependency(filterOp)
        
        photoFetchQueue.addOperation(fetchOp)
        photoFetchQueue.addOperation(cacheOp)
        imageFilteringQueue.addOperation(filterOp) //Add operation on background queue
